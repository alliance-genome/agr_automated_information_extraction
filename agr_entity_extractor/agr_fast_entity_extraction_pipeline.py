...
Strain            \b(?=.*[A-Za-z])(?=.*\d)[A-Za-z0-9:_\-]+\b    Must contain both a letter and a digit
Gene/Transgenic   \b[A-Za-z0-9:_.\-]{2,}\b                      At least 2 characters; allows letters, digits, 
                                                                :, _, ., -
                                                                Uses word boundaries to capture names like 
                                                                "kyEx926" in kyEx926(Punc-86::MIG-10::YFP)
                                                                correctly
"""
...
STRAIN_NAME_PATTERN = re.compile(
    r'(?<!\S)'                              # left boundary: start or whitespace
    r'(?=[A-Za-z0-9:()_\-]*[A-Za-z])'       # inside the token: at least one letter
    r'(?=[A-Za-z0-9:()_\-]*\d)'             # inside the token: at least one digit
    r'[A-Za-z0-9:()_\-]+'                   # the token (now including parentheses)
    r'(?!\S)'                               # right boundary: whitespace or end
)
STRAIN_NAME_PATTERN = re.compile(
    r'(?<!\S)'                              # left-boundary: start or whitespace
    r'(?=[A-Za-z0-9()_\-]*[A-Za-z])'        # must contain ≥1 letter
    r'(?=[A-Za-z0-9()_\-]*\d)'              # must contain ≥1 digit
    r'[A-Za-z0-9()_\-]+'                    # the token itself (no colon)
    r'(?=[\s:;,\.]|$)'                      # right-boundary: whitespace, colon, semicolon, comma, period, or end
)
"""
STRAIN_NAME_PATTERN = re.compile(
    r'''
    ...
    (?:                           # then any of…
     ...
     | \([A-Za-z0-9]+\)           #   or a balanced "(…)" group of alphanumerics
