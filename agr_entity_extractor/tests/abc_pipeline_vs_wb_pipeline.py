...
def load_abc_file(filepath: str) -> Dict[str, Dict[str, Set[str]]]:
    entity_data = {}
    with open(filepath, 'r') as f:
        for line in f:
            ...
            # Normalize and uppercase entity names
            entities = set(e.strip().upper() for e in entity_str.split('|') if e.strip())
            entity_data[mod_id] = {"agrkb_id": agrkb_id, "entities": entities}
    return entity_data


def load_baseline_file(filepath: str) -> Dict[str, Set[str]]:
    entity_data = {}
    with open(filepath, 'r') as f:
        for line in f:
            parts = line.strip().split('\t')
            ...
            if len(parts) != 3:
                continue
            mod_id, entity_str, datestamp = parts
            if not datestamp.startswith("2025-") and not datestamp.startswith("2024-1"):
                continue
            """
            mod_id, entity_str = parts
            ...
            for part in entity_str.split('|'):
                ...
                if ';%;' in token:
                    ...
                    name = token
                entities.add(name.strip().upper())
            entity_data[mod_id] = entities
    return entity_data
...
def compare_entities(
    ...
    for mod_id in shared_ids:
        agrkb_id = abc_data[mod_id]["agrkb_id"]
        abc_entities = abc_data[mod_id]["entities"]
        baseline_entities = baseline_data[mod_id]
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python compare_entity_extractions.py <entity_type> abc_file baseline_file")
        sys.exit(1)
