...
# Strain validation configuration
STRAIN_VALIDATION = {
    'min_length': 2,
    'max_length': 50,
    'allowed_chars': r'^[a-zA-Z0-9\/\-_\.]+$',
    'blacklist': {'unknown', 'wildtype', 'WT', 'control', 'strain', 'mixed'},
    'require_alphanum': True,
...
def validate_strain(entity: str) -> bool:
    """Validate strain names using configurable rules"""
    if not entity:
        return False
    ...
    # Length validation
    if len(clean_entity) < STRAIN_VALIDATION['min_length']:
        return False
    if len(clean_entity) > STRAIN_VALIDATION['max_length']:
        return False
    
    # Blacklist check
    if clean_entity.lower() in STRAIN_VALIDATION['blacklist']:
        return False
    
    # Character validation
    if not re.match(STRAIN_VALIDATION['allowed_chars'], clean_entity):
        return False
    
    # Alphanumeric requirement
    if STRAIN_VALIDATION['require_alphanum'] and not any(c.isalnum() for c in clean_entity):
        return False
...
def process_entity_extraction_jobs(mod_id, topic, jobs):
    ...
    tet_source_id = get_tet_source_id(mod_abbreviation=mod_abbr,
                                    ...
                                    source_description="Alliance entity extraction pipeline")

    entity_extraction_model_file_path = (
        f"/data/agr_document_classifier/biocuration_entity_extraction_{mod_abbr}_"
        f"{topic.replace(':', '_')}.dpkl"
    )

    try:
        model_metadata = get_model_data(mod_abbreviation=mod_abbr,
                                      task_type="biocuration_entity_extraction",
                                      topic=topic)
        ...
        download_abc_model(mod_abbreviation=mod_abbr, topic=topic,
                          output_path=entity_extraction_model_file_path,
                          task_type="biocuration_entity_extraction")
        logger.info(f"Model downloaded for {mod_abbr}/{topic}")
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            logger.warning(f"Model not found for {mod_abbr}/{topic}")
            return
        raise
    ...
    while jobs_to_process:
        ...
        # Load fresh model instance per batch
        with open(entity_extraction_model_file_path, "rb") as f:
            entity_extraction_model = dill.load(f)

        # Strain-specific initialization
        if topic == 'ATP:0000027':
            entity_extraction_model.load_entities_dynamically()
            if hasattr(entity_extraction_model.vectorizer, 'partial_fit'):
                entity_extraction_model.vectorizer.n_features_ = 0
                entity_extraction_model.vectorizer.vocabulary_ = {}

        # Clean download directory
        os.makedirs("/data/agr_entity_extraction/to_extract", exist_ok=True)
        for f in os.listdir("/data/agr_entity_extraction/to_extract"):
            os.remove(os.path.join("/data/agr_entity_extraction/to_extract", f))
        ...
        for file in os.listdir("/data/agr_entity_extraction/to_extract"):
            curie = file.split(".")[0].replace("_", ":")
            job = reference_curie_job_map.get(curie)
            if not job:
                continue
            ...
            try:
                ...
                # Create fresh pipeline per document
                ...
                fulltext = tei_obj.get_fulltext()
                title = tei_obj.get_title() or ""
                abstract = tei_obj.get_abstract() or ""
                ...
                """
                # Apply strain validation
                if topic == 'ATP:0000027':
                    entities = {e for e in entities if validate_strain(e)}
                """
                ...
                if not entities:
                    ...
                    for entity in entities:
                        ...
                        lookup_entity = entity.upper() if not STRAIN_VALIDATION['case_sensitive'] else entity
                        entity_curie = entity_extraction_model.name_to_curie_mapping.get(
                            lookup_entity,
                            entity_extraction_model.upper_to_original_mapping.get(lookup_entity, None)
                        )
                        if entity_curie:
                            send_entity_tag_to_abc(
                                reference_curie=curie,
                    ...
                    """
                    print(f"{curie}:", entities)
                set_job_started(job)
                set_job_success(job)

            except Exception as e:
                logger.error(f"Error processing {curie}: {str(e)}")
                set_job_failure(job)
            finally:
                del tei_obj
                del nlp_pipeline
        ...
        del entity_extraction_model
        gc.collect()

def extract_all_entities(nlp_pipeline, fulltext, entity_extraction_model, title, abstract, topic):
    """Entity extraction with manual frequency management"""
    results = nlp_pipeline(fulltext)
    entities_in_fulltext = [result['word'] for result in results if result['entity'] == "ENTITY"]
    
    # Case handling configuration
    case_sensitive = STRAIN_VALIDATION['case_sensitive'] if topic == 'ATP:0000027' else True
    
    # Manual frequency tracking
    entity_counts = {}
    def update_counts(entity):
        key = entity if case_sensitive else entity.upper()
        entity_counts[key] = entity_counts.get(key, 0) + 1

    # Process all text sources
    for entity in entities_in_fulltext:
        update_counts(entity)
    for token in entity_extraction_model.tokenizer.tokenize(title):
        update_counts(token)
    for token in entity_extraction_model.tokenizer.tokenize(abstract):
        update_counts(token)
    ...
    final_entities = []
    for entity in entities_in_fulltext + \
                  entity_extraction_model.tokenizer.tokenize(title) + \
                  entity_extraction_model.tokenizer.tokenize(abstract):
        key = entity if case_sensitive else entity.upper()
        if key in entity_counts and key not in seen:
            final_entities.append(entity)
            seen.add(key)
    
    return list(set(final_entities))  # Return unique entities
